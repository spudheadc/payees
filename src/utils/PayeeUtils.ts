...
export const convertToBiller = (payee: PayeeData | undefined, event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>): PayeeData => {
    if (!payee)
        payee = initialisePayee();

    let biller: Biller = initialiseBiller(payee);
...
export const convertToType = (payee: PayeeData | undefined, event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>): PayeeData => {
    if (!payee)
        payee = initialisePayee();
...
export const convertToBsbAccount = (payee: PayeeData | undefined, event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>): PayeeData => {
    if (!payee)
        payee = initialisePayee();
    let account: Account = initialiseAccount(payee);
...
export const convertToPayId = (payee: PayeeData | undefined, event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>): PayeeData => {
    if (!payee)
        payee = initialisePayee();
    let payId: DigitalWallet = initialisePayId(payee);
...
function initialisePayId(payee: PayeeData): DigitalWallet {
    ...
    if (!payee.domestic)
        payee.domestic = { payeeAccountUType: "" };
...
function initialiseAccount(payee: PayeeData): Account {
    ...
    if (!payee.domestic.account)
        payee.domestic.account = { accountName: "", bsb: "", accountNumber: "" };
