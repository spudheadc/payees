...
import { Biller, PayeeData } from '../../types/Payee';
...
const BpayPayee = ({ setBiller, biller, editable = false }: ChildPropsType): JSX.Element => {
        ...
        return (<div className="mb-3">
            ...
            {editable ?
                <input
                    ...
                    autoComplete={field}
                    placeholder={label}
                    defaultValue={biller ? biller[field as keyof Biller] : ""}
                    ...
                    }} /> : <label className="w-full bg-white">{biller ? biller[field as keyof Biller] : ""}</label>}
        </div>);
    };

    const NonEditableField = ({ field, label }: { field: string, label: string }): JSX.Element => {
        return (<div className="mb-3">
            <label htmlFor={field} className="sr-only">
                {label}
            </label>
