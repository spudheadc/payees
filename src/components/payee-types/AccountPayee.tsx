...
const AccountPayee = ({ setAccount, account, editable = false }: ChildPropsType): JSX.Element => {
        ...
        return (<div className="mb-3">
            ...
            {editable ?
                <input
                    ...
                    autoComplete={field}
                    placeholder={label}
                    defaultValue = {account ? account[field as keyof Account] : ""}
                    ...
                    }} /> : <label className="w-full bg-white">{account ? account[field as keyof Account] : ""}</label>}
        </div>);
    };

    const EditableBSB = ({ field, label }: { field: string, label: string }): JSX.Element => {
        return (<div className="mb-3">
            <label htmlFor={field} className="sr-only">
                {label}
            </label>
